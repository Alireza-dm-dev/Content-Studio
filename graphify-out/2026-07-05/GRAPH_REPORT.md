# Graph Report - content-studio  (2026-07-05)

## Corpus Check
- 243 files · ~4,314,648 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3005 nodes · 5751 edges · 184 communities (158 shown, 26 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 153 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6070bc41`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_Live Browser Core|Live Browser Core]]
- [[_COMMUNITY_Live Server Core|Live Server Core]]
- [[_COMMUNITY_CSS Rule Checks|CSS Rule Checks]]
- [[_COMMUNITY_Live Browser UI Components|Live Browser UI Components]]
- [[_COMMUNITY_Injected Browser Checks|Injected Browser Checks]]
- [[_COMMUNITY_Screenshot & Bundled Runtime|Screenshot & Bundled Runtime]]
- [[_COMMUNITY_Browser URL Detector|Browser URL Detector]]
- [[_COMMUNITY_Live Manual Edit Commit|Live Manual Edit Commit]]
- [[_COMMUNITY_Editing State Machine|Editing State Machine]]
- [[_COMMUNITY_Live Svelte Component System|Live Svelte Component System]]
- [[_COMMUNITY_Generated File Detection|Generated File Detection]]
- [[_COMMUNITY_Content Calendar Page|Content Calendar Page]]
- [[_COMMUNITY_Package Dependencies|Package Dependencies]]
- [[_COMMUNITY_UI Utils & Badge Components|UI Utils & Badge Components]]
- [[_COMMUNITY_Live Browser Session Management|Live Browser Session Management]]
- [[_COMMUNITY_Browser Anti-Pattern Detection|Browser Anti-Pattern Detection]]
- [[_COMMUNITY_CSS Element Quality Rules|CSS Element Quality Rules]]
- [[_COMMUNITY_Live Event Validation|Live Event Validation]]
- [[_COMMUNITY_Design Parser|Design Parser]]
- [[_COMMUNITY_Live Accept Module|Live Accept Module]]
- [[_COMMUNITY_Live Browser DOM Editing|Live Browser DOM Editing]]
- [[_COMMUNITY_Live Browser UI Rendering|Live Browser UI Rendering]]
- [[_COMMUNITY_Copy Edit Agent|Copy Edit Agent]]
- [[_COMMUNITY_Live Browser Focus & Steer|Live Browser Focus & Steer]]
- [[_COMMUNITY_Live Browser Param Management|Live Browser Param Management]]
- [[_COMMUNITY_Live Browser Clipboard & State|Live Browser Clipboard & State]]
- [[_COMMUNITY_Content Studio Project Docs|Content Studio Project Docs]]
- [[_COMMUNITY_Manual Edit Evidence Analysis|Manual Edit Evidence Analysis]]
- [[_COMMUNITY_Agent Skills & Design Tools|Agent Skills & Design Tools]]
- [[_COMMUNITY_CLI Main & URL Detection|CLI Main & URL Detection]]
- [[_COMMUNITY_Live Completion & Poll|Live Completion & Poll]]
- [[_COMMUNITY_Visual Contrast Analysis|Visual Contrast Analysis]]
- [[_COMMUNITY_Brand Detail Page|Brand Detail Page]]
- [[_COMMUNITY_Component Library|Component Library]]
- [[_COMMUNITY_Live Inject Module|Live Inject Module]]
- [[_COMMUNITY_Impeccable Paths & Live Complete|Impeccable Paths & Live Complete]]
- [[_COMMUNITY_Static HTMLCSS Cascade|Static HTML/CSS Cascade]]
- [[_COMMUNITY_Video Creation Page|Video Creation Page]]
- [[_COMMUNITY_Live Discard Manual Edits|Live Discard Manual Edits]]
- [[_COMMUNITY_Context Builder|Context Builder]]
- [[_COMMUNITY_AI Analysis API|AI Analysis API]]
- [[_COMMUNITY_Color Anti-Pattern Checks|Color Anti-Pattern Checks]]
- [[_COMMUNITY_Calendar Post Utilities|Calendar Post Utilities]]
- [[_COMMUNITY_Impeccable Path Utils|Impeccable Path Utils]]
- [[_COMMUNITY_Export & Google Drive API|Export & Google Drive API]]
- [[_COMMUNITY_Color Detection Utils|Color Detection Utils]]
- [[_COMMUNITY_Brand Identity Utils|Brand Identity Utils]]
- [[_COMMUNITY_SvelteKit Adapter|SvelteKit Adapter]]
- [[_COMMUNITY_Post Regeneration API|Post Regeneration API]]
- [[_COMMUNITY_Post Regeneration (Legacy)|Post Regeneration (Legacy)]]
- [[_COMMUNITY_Calendar Post CRUD & Image Prompt|Calendar Post CRUD & Image Prompt]]
- [[_COMMUNITY_Eyebrow & H1 Anti-Pattern Checks|Eyebrow & H1 Anti-Pattern Checks]]
- [[_COMMUNITY_Context Signals|Context Signals]]
- [[_COMMUNITY_Critique Storage|Critique Storage]]
- [[_COMMUNITY_Calendar Detail Page|Calendar Detail Page]]
- [[_COMMUNITY_CSS Typography Rules|CSS Typography Rules]]
- [[_COMMUNITY_Cleanup Deprecated|Cleanup Deprecated]]
- [[_COMMUNITY_Live Session Store|Live Session Store]]
- [[_COMMUNITY_Brand Workspace Page|Brand Workspace Page]]
- [[_COMMUNITY_Brand Identity Extraction|Brand Identity Extraction]]
- [[_COMMUNITY_Image Generation & Higgsfield|Image Generation & Higgsfield]]
- [[_COMMUNITY_Overflow & Clipping Checks|Overflow & Clipping Checks]]
- [[_COMMUNITY_CSP Detection|CSP Detection]]
- [[_COMMUNITY_Voice Interface|Voice Interface]]
- [[_COMMUNITY_Live Server Event Management|Live Server Event Management]]
- [[_COMMUNITY_Palette Builder|Palette Builder]]
- [[_COMMUNITY_Project Root & Pin|Project Root & Pin]]
- [[_COMMUNITY_Content Calendar AI Generation|Content Calendar AI Generation]]
- [[_COMMUNITY_Glow & Palette Anti-Patterns|Glow & Palette Anti-Patterns]]
- [[_COMMUNITY_Video From Brand Page|Video From Brand Page]]
- [[_COMMUNITY_Live UI Core|Live UI Core]]
- [[_COMMUNITY_Settings & AI Test Card|Settings & AI Test Card]]
- [[_COMMUNITY_Video Storyboard Modal|Video Storyboard Modal]]
- [[_COMMUNITY_Browser Findings Overlay|Browser Findings Overlay]]
- [[_COMMUNITY_Live Entry Point|Live Entry Point]]
- [[_COMMUNITY_Live Server Broadcast|Live Server Broadcast]]
- [[_COMMUNITY_Post Suggestion API|Post Suggestion API]]
- [[_COMMUNITY_Video Prompt Modal|Video Prompt Modal]]
- [[_COMMUNITY_Generated Prompts Page|Generated Prompts Page]]
- [[_COMMUNITY_Brand Identity Report|Brand Identity Report]]
- [[_COMMUNITY_Module Group 80|Module Group 80]]
- [[_COMMUNITY_Module Group 81|Module Group 81]]
- [[_COMMUNITY_Module Group 82|Module Group 82]]
- [[_COMMUNITY_Module Group 83|Module Group 83]]
- [[_COMMUNITY_Module Group 84|Module Group 84]]
- [[_COMMUNITY_Module Group 85|Module Group 85]]
- [[_COMMUNITY_Module Group 86|Module Group 86]]
- [[_COMMUNITY_Module Group 87|Module Group 87]]
- [[_COMMUNITY_Module Group 88|Module Group 88]]
- [[_COMMUNITY_Module Group 89|Module Group 89]]
- [[_COMMUNITY_Module Group 90|Module Group 90]]
- [[_COMMUNITY_Module Group 91|Module Group 91]]
- [[_COMMUNITY_Module Group 92|Module Group 92]]
- [[_COMMUNITY_Module Group 93|Module Group 93]]
- [[_COMMUNITY_Module Group 94|Module Group 94]]
- [[_COMMUNITY_Module Group 95|Module Group 95]]
- [[_COMMUNITY_Module Group 96|Module Group 96]]
- [[_COMMUNITY_Module Group 97|Module Group 97]]
- [[_COMMUNITY_Module Group 98|Module Group 98]]
- [[_COMMUNITY_Module Group 99|Module Group 99]]
- [[_COMMUNITY_Module Group 100|Module Group 100]]
- [[_COMMUNITY_Module Group 101|Module Group 101]]
- [[_COMMUNITY_Module Group 102|Module Group 102]]
- [[_COMMUNITY_Module Group 103|Module Group 103]]
- [[_COMMUNITY_Module Group 104|Module Group 104]]
- [[_COMMUNITY_Module Group 105|Module Group 105]]
- [[_COMMUNITY_Module Group 106|Module Group 106]]
- [[_COMMUNITY_Module Group 108|Module Group 108]]
- [[_COMMUNITY_Module Group 109|Module Group 109]]
- [[_COMMUNITY_Module Group 110|Module Group 110]]
- [[_COMMUNITY_Module Group 111|Module Group 111]]
- [[_COMMUNITY_Module Group 112|Module Group 112]]
- [[_COMMUNITY_Module Group 113|Module Group 113]]
- [[_COMMUNITY_Module Group 114|Module Group 114]]
- [[_COMMUNITY_Module Group 115|Module Group 115]]
- [[_COMMUNITY_Module Group 116|Module Group 116]]
- [[_COMMUNITY_Module Group 117|Module Group 117]]
- [[_COMMUNITY_Module Group 118|Module Group 118]]
- [[_COMMUNITY_Module Group 119|Module Group 119]]
- [[_COMMUNITY_Module Group 120|Module Group 120]]
- [[_COMMUNITY_Module Group 121|Module Group 121]]
- [[_COMMUNITY_Module Group 122|Module Group 122]]
- [[_COMMUNITY_Module Group 123|Module Group 123]]
- [[_COMMUNITY_Module Group 124|Module Group 124]]
- [[_COMMUNITY_Module Group 125|Module Group 125]]
- [[_COMMUNITY_Module Group 126|Module Group 126]]
- [[_COMMUNITY_Module Group 127|Module Group 127]]
- [[_COMMUNITY_Module Group 128|Module Group 128]]
- [[_COMMUNITY_Module Group 129|Module Group 129]]
- [[_COMMUNITY_Module Group 130|Module Group 130]]
- [[_COMMUNITY_Module Group 131|Module Group 131]]
- [[_COMMUNITY_Module Group 132|Module Group 132]]
- [[_COMMUNITY_Module Group 133|Module Group 133]]
- [[_COMMUNITY_Module Group 134|Module Group 134]]
- [[_COMMUNITY_Module Group 135|Module Group 135]]
- [[_COMMUNITY_Module Group 136|Module Group 136]]
- [[_COMMUNITY_Module Group 137|Module Group 137]]
- [[_COMMUNITY_Module Group 138|Module Group 138]]
- [[_COMMUNITY_Module Group 139|Module Group 139]]
- [[_COMMUNITY_Module Group 140|Module Group 140]]
- [[_COMMUNITY_Module Group 141|Module Group 141]]
- [[_COMMUNITY_Module Group 142|Module Group 142]]
- [[_COMMUNITY_Module Group 143|Module Group 143]]
- [[_COMMUNITY_Module Group 144|Module Group 144]]
- [[_COMMUNITY_Module Group 145|Module Group 145]]
- [[_COMMUNITY_Module Group 146|Module Group 146]]
- [[_COMMUNITY_Module Group 147|Module Group 147]]
- [[_COMMUNITY_Module Group 148|Module Group 148]]
- [[_COMMUNITY_Module Group 149|Module Group 149]]
- [[_COMMUNITY_Module Group 150|Module Group 150]]
- [[_COMMUNITY_Module Group 151|Module Group 151]]
- [[_COMMUNITY_Module Group 152|Module Group 152]]
- [[_COMMUNITY_Module Group 153|Module Group 153]]
- [[_COMMUNITY_Module Group 154|Module Group 154]]
- [[_COMMUNITY_Module Group 155|Module Group 155]]
- [[_COMMUNITY_Module Group 156|Module Group 156]]
- [[_COMMUNITY_Module Group 157|Module Group 157]]
- [[_COMMUNITY_Module Group 159|Module Group 159]]
- [[_COMMUNITY_Module Group 160|Module Group 160]]
- [[_COMMUNITY_Module Group 161|Module Group 161]]
- [[_COMMUNITY_Community 162|Community 162]]
- [[_COMMUNITY_Community 163|Community 163]]
- [[_COMMUNITY_Community 164|Community 164]]
- [[_COMMUNITY_Community 166|Community 166]]
- [[_COMMUNITY_Community 167|Community 167]]
- [[_COMMUNITY_Community 168|Community 168]]
- [[_COMMUNITY_Community 169|Community 169]]
- [[_COMMUNITY_Community 170|Community 170]]
- [[_COMMUNITY_Community 171|Community 171]]
- [[_COMMUNITY_Community 172|Community 172]]
- [[_COMMUNITY_Community 173|Community 173]]
- [[_COMMUNITY_Community 174|Community 174]]
- [[_COMMUNITY_Community 175|Community 175]]
- [[_COMMUNITY_Community 176|Community 176]]
- [[_COMMUNITY_Community 177|Community 177]]
- [[_COMMUNITY_Community 178|Community 178]]
- [[_COMMUNITY_Community 179|Community 179]]
- [[_COMMUNITY_Community 180|Community 180]]
- [[_COMMUNITY_Community 181|Community 181]]
- [[_COMMUNITY_Community 182|Community 182]]
- [[_COMMUNITY_Community 183|Community 183]]

## God Nodes (most connected - your core abstractions)
1. `handleClick()` - 26 edges
2. `handleKeyDown()` - 26 edges
3. `cn()` - 26 edges
4. `commitManualEdits()` - 24 edges
5. `collectBrowserFindings()` - 23 edges
6. `generateWithPromptTemplate()` - 23 edges
7. `Conceptual Sketch — Design System` - 23 edges
8. `injectSvelteComponentsFromManifest()` - 22 edges
9. `showToast()` - 22 edges
10. `resumeSession()` - 22 edges

## Surprising Connections (you probably didn't know these)
- `SplitIdentityView()` --calls--> `normalizeBrandIdentityOutput()`  [INFERRED]
  app/brands/[id]/extract-identity/ExtractIdentityClient.jsx → lib/brand-identity-utils.js
- `ImagePromptModal()` --calls--> `formatOutputImageTextRequirementsForDisplay()`  [INFERRED]
  components/ImagePromptModal.jsx → lib/calendar-post-utils.js
- `Input()` --calls--> `cn()`  [INFERRED]
  components/ui/input.jsx → lib/utils.js
- `Label()` --calls--> `cn()`  [INFERRED]
  components/ui/label.jsx → lib/utils.js
- `Separator()` --calls--> `cn()`  [INFERRED]
  components/ui/separator.jsx → lib/utils.js

## Import Cycles
- 1-file cycle: `components/ui/sonner.jsx -> components/ui/sonner.jsx`
- 1-file cycle: `components/ui/button.jsx -> components/ui/button.jsx`
- 1-file cycle: `components/ui/separator.jsx -> components/ui/separator.jsx`
- 1-file cycle: `components/ui/input.jsx -> components/ui/input.jsx`
- 1-file cycle: `components/ui/select.jsx -> components/ui/select.jsx`

## Hyperedges (group relationships)
- **Design Handoff Template Suite** — agents_skills_frontend_design_ui_ux_references_component_spec_template, agents_skills_frontend_design_ui_ux_references_design_tokens_template, agents_skills_frontend_design_ui_ux_references_user_flow_template [EXTRACTED 1.00]
- **Impeccable Project Setup Flow** — agents_skills_impeccable_reference_init, agents_skills_impeccable_reference_document, agents_skills_impeccable_reference_live [EXTRACTED 1.00]
- **Impeccable Craft Visual Direction Pipeline** — agents_skills_impeccable_reference_craft, agents_skills_impeccable_reference_codex, agents_skills_impeccable_reference_brand [EXTRACTED 1.00]
- **Impeccable Commands That Funnel into Polish as Final Step** — agents_skills_impeccable_reference_onboard, agents_skills_impeccable_reference_optimize, agents_skills_impeccable_reference_quieter, agents_skills_impeccable_reference_typeset, agents_skills_impeccable_reference_polish, agents_skills_impeccable_reference_polish_polish_handoff_pattern [EXTRACTED 1.00]
- **Content Studio Project Documentation Suite** — docs_architecture, docs_decisions, docs_design_system, docs_features, docs_project_context_for_ai, docs_roadmap [INFERRED 0.95]
- **Conceptual Sketch Design System Core: Spec, State Audit, and Governing Principles** — content_studio_design_system, docs_design_system, content_studio_design_system_sketch_identity, content_studio_design_system_single_accent_rule, content_studio_design_system_sketch_tokens [INFERRED 0.85]

## Communities (184 total, 26 thin omitted)

### Community 0 - "Live Browser Core"
Cohesion: 0.04
Nodes (90): acceptedDomAlreadyClean(), applyPlaceholderDimensions(), applyPlaceholderSizingStyles(), averageRgb01(), beginEditPin(), buildAnnotationsForCapture(), buildPinElement(), buildSvelteExpressionTextMap() (+82 more)

### Community 1 - "Live Server Core"
Cohesion: 0.04
Nodes (74): acknowledgePendingEvent(), addOpToManualApplyChunk(), agentPollingConnected(), annotRoot, APPLY_EVENT_HARD_TIMEOUT_MS, APPLY_EVENT_SOFT_DEADLINE_MS, args, broadcast() (+66 more)

### Community 2 - "CSS Rule Checks"
Cohesion: 0.06
Nodes (62): borderWidthsFromStyle(), checkBorders(), checkClippedOverflow(), checkElementBorders(), checkElementBordersDOM(), checkElementClippedOverflow(), checkElementClippedOverflowDOM(), checkElementGptBorderShadow() (+54 more)

### Community 3 - "Live Browser UI Components"
Cohesion: 0.07
Nodes (51): addBrowserFindings(), addVisualContrastFindings(), addVisualContrastResult(), analyzeVisualContrast(), analyzeVisualContrastCandidate(), blendRgba(), browserFindingsFromMap(), buildSelectorSegment() (+43 more)

### Community 4 - "Injected Browser Checks"
Cohesion: 0.08
Nodes (55): actionLabel(), attachSteerFocusGuard(), barPaletteForTheme(), brandMarkSvg(), buildConfigureRow(), buildConfirmedRow(), buildCyclingRow(), buildDots() (+47 more)

### Community 5 - "Screenshot & Bundled Runtime"
Cohesion: 0.11
Nodes (53): ae(), be(), bt(), Ce(), Ct(), de(), dt(), _e() (+45 more)

### Community 6 - "Browser URL Detector"
Cohesion: 0.09
Nodes (37): detectUrl(), runVisualContrastFallback(), CSS_IN_JS_EXTENSIONS, detectText(), extractCSSinJS(), extractStyleBlocks(), REGEX_ANALYZERS, REGEX_MATCHERS (+29 more)

### Community 7 - "Live Manual Edit Commit"
Cohesion: 0.11
Nodes (50): cancelEditing(), cancelEditingToPicking(), cancelInsertConfigure(), clearAnnotations(), clearInsertPicking(), closeTunePopover(), cursorForInsertAxis(), cycleVariant() (+42 more)

### Community 8 - "Editing State Machine"
Cohesion: 0.09
Nodes (50): allEntryIds(), argVal(), buildRepairBatch(), candidatesForEntry(), changedFilesSinceSnapshot(), clearAppliedEntries(), collectApplyOwnedFiles(), collectRollbackFiles() (+42 more)

### Community 9 - "Live Svelte Component System"
Cohesion: 0.09
Nodes (45): abortSvelteComponentInjection(), applySavedSessionMeta(), buildInsertPlaceholderSnapshotFromDom(), captureAndEmit(), checkpointPayload(), clampVariantIndex(), cleanup(), cleanupAcceptedSession() (+37 more)

### Community 10 - "Generated File Detection"
Cohesion: 0.10
Nodes (43): applyLegacyDeferredAcceptsOnStartup(), appendCssToSvelteStyle(), appendSanitizedCssRule(), applyDeferredSvelteComponentAccepts(), bakeParamValuesInCss(), buildInsertVariantStub(), buildPropContract(), buildPropsScript() (+35 more)

### Community 11 - "Content Calendar Page"
Cohesion: 0.12
Nodes (39): hasGeneratedHeader(), HEADER_MARKERS, isGeneratedFile(), isGitIgnored(), argVal(), buildInsertWrapperLines(), computeInsertLine(), INSERT_POSITIONS (+31 more)

### Community 12 - "Package Dependencies"
Cohesion: 0.05
Nodes (41): README.md - Project README (generic create-next-app boilerplate), dependencies, @base-ui/react, better-sqlite3, class-variance-authority, clsx, geist, @higgsfield/client (+33 more)

### Community 13 - "UI Utils & Badge Components"
Cohesion: 0.11
Nodes (22): options, STATUS_CONFIG, Card(), CardAction(), CardContent(), CardDescription(), CardFooter(), CardHeader() (+14 more)

### Community 14 - "Live Browser Session Management"
Cohesion: 0.09
Nodes (40): activeElementDeep(), attachSteerFocusDebug(), buildSteerProcessingDots(), clearSteerAwaitTimer(), collapsePageChat(), expandPageChat(), finishVoiceSession(), focusPageChatInput() (+32 more)

### Community 15 - "Browser Anti-Pattern Detection"
Cohesion: 0.08
Nodes (29): buildSelectorSegment(), checkBorders(), checkElementBorders(), checkElementBordersDOM(), checkElementItalicSerif(), checkElementItalicSerifDOM(), checkElementMotion(), checkElementMotionDOM() (+21 more)

### Community 16 - "CSS Element Quality Rules"
Cohesion: 0.08
Nodes (23): FORBIDDEN_MANUAL_EDIT_TEXT_CHARS, INSERT_POSITIONS, isValidId(), isValidVariantId(), validateAnnotationFields(), validateEvent(), validateInsertGenerate(), validateManualEditEvent() (+15 more)

### Community 17 - "Live Event Validation"
Cohesion: 0.15
Nodes (32): buildColor(), CANONICAL_SECTIONS, collectBullets(), collectColorValues(), collectParagraphs(), detectFormat(), extractColors(), extractComponents() (+24 more)

### Community 18 - "Design Parser"
Cohesion: 0.06
Nodes (32): Accent Color Application, Accessibility, Alpha Is A Design Smell, Assess Color Opportunity, Background & Surfaces, Balance & Refinement, Borders & Accents, Building Functional Palettes (+24 more)

### Community 19 - "Live Accept Module"
Cohesion: 0.13
Nodes (30): acceptCli(), argVal(), buildCarbonizeReplacement(), decodeHtmlAttr(), deindentContent(), detectCommentSyntax(), escapeRegExp(), expandReplaceRange() (+22 more)

### Community 20 - "Live Browser DOM Editing"
Cohesion: 0.09
Nodes (33): addManualContextText(), applyEditing(), buildLocatorForLeaf(), canRestoreManualEditElement(), collectManualContextPieces(), contextElementForManualEdit(), copyEditContainerContext(), copyEditLeafContext() (+25 more)

### Community 21 - "Live Browser UI Rendering"
Cohesion: 0.13
Nodes (27): detectCli(), formatFindings(), handleStdin(), printUsage(), createBrowserDetector(), buildImportGraph(), detectFrameworkConfig(), FRAMEWORK_CONFIGS (+19 more)

### Community 22 - "Copy Edit Agent"
Cohesion: 0.09
Nodes (32): buildCollapsible(), buildColorModels(), buildDesignHeader(), buildRadiiModels(), buildTypographyModels(), cssSafe(), escapeHtml(), fetchDesignSystem() (+24 more)

### Community 23 - "Live Browser Focus & Steer"
Cohesion: 0.13
Nodes (30): applyMockWrites(), buildCopyEditBatchPrompt(), checkFrameworkSourceSyntax(), chooseCopyEditAgent(), COMMAND_AUTH_CACHE, commandAuthed(), commandExists(), compactBatchForPrompt() (+22 more)

### Community 24 - "Live Browser Param Management"
Cohesion: 0.15
Nodes (30): confirm(), clearStoredManualApplyState(), copyToClipboard(), fetchPendingCount(), handleManualEditActivity(), hidePendingApplyDock(), manualApplyLoadingText(), manualApplyStateKey() (+22 more)

### Community 25 - "Live Browser Clipboard & State"
Cohesion: 0.12
Nodes (23): applyStaticDeclaration(), buildBorderOverrideMap(), compareStaticPriority(), cssPropToCamel(), expandStaticBoxValues(), expandStaticDeclaration(), extractStaticColor(), makeStaticStyle() (+15 more)

### Community 26 - "Content Studio Project Docs"
Cohesion: 0.09
Nodes (22): Assess Onboarding Needs, Context Over Ceremony, Contextual Help, Design Onboarding Experiences, Documentation & Help, Empty State Design, Feature Discovery & Adoption, Guided Tours & Walkthroughs (+14 more)

### Community 27 - "Manual Edit Evidence Analysis"
Cohesion: 0.10
Nodes (29): applyOriginalAttrsToSvelteAnchor(), applyParamDefaults(), applyParamValue(), closedClipPath(), commitAcceptedSvelteComponentToDom(), getMountedSvelteComponentAnchor(), getVisibleVariantEl(), hideParamsPanel() (+21 more)

### Community 28 - "Agent Skills & Design Tools"
Cohesion: 0.22
Nodes (9): After This File, Codex: Visual Direction & Asset Production, Four stop points before code, Step A: Explore Directions with the User, Step B: Generate the Brand Palette First, Step C: Generate 1-3 Visual Mocks Against the Palette, Step D: Approval Loop, Step E: Mock Fidelity Inventory (+1 more)

### Community 29 - "CLI Main & URL Detection"
Cohesion: 0.15
Nodes (25): classifyFetchError(), errorResponse(), POST(), ALLOWED_FORMATS, buildIdentitySummary(), classifyFetchError(), errorResponse(), normaliseLinkedInPost() (+17 more)

### Community 30 - "Live Completion & Poll"
Cohesion: 0.07
Nodes (27): Animations, Border Radius, Brand Colors, Breakpoints, Colors, Container Widths, CSS Variables Export, Dark Mode Overrides (+19 more)

### Community 31 - "Visual Contrast Analysis"
Cohesion: 0.16
Nodes (27): checkColors(), checkCreamPalette(), checkElementAIPaletteDOM(), checkElementColors(), checkElementColorsDOM(), checkElementGlow(), checkElementGlowDOM(), checkElementIconTile() (+19 more)

### Community 32 - "Brand Detail Page"
Cohesion: 0.14
Nodes (24): analyzeSourceHint(), buildCandidatesForOp(), buildContextHintsByRef(), collectSearchFiles(), countOps(), escapeRegExp(), findContextMatches(), findLiteralMatches() (+16 more)

### Community 33 - "Component Library"
Cohesion: 0.09
Nodes (21): aliases, components, hooks, lib, ui, utils, iconLibrary, menuAccent (+13 more)

### Community 34 - "Live Inject Module"
Cohesion: 0.08
Nodes (24): Component translation rules, Narrative mapping, Pitfalls, Scan mode (approach C: auto-extract, then confirm descriptive language), Schema, Seed mode, Step 1: Confirm seed mode, Step 1: Find the design assets (+16 more)

### Community 35 - "Impeccable Paths & Live Complete"
Cohesion: 0.08
Nodes (24): 10. Card styles, 11. Modal styles, 12. Table styles, 13. Navigation and sidebar style, 14. Page header style, 15. Empty state style, 16. Badge and tag style, 17. Toast and alert style (+16 more)

### Community 36 - "Static HTML/CSS Cascade"
Cohesion: 0.18
Nodes (22): completionAckForAcceptResult(), completionTypeForAcceptResult(), augmentEventWithAcceptHandling(), buildAcceptScriptArgs(), buildPollReplyPayload(), EVENT_TYPES_NEEDING_AGENT_REPLY, fetchNextEvent(), fetchServerStatus() (+14 more)

### Community 37 - "Video Creation Page"
Cohesion: 0.09
Nodes (22): Acceptance Criteria, Accessibility Considerations, Analytics Events, Cancel/Abandon Exit, Decision: [Decision Name], Decision Points, Edge Cases, Entry Points (+14 more)

### Community 38 - "Live Discard Manual Edits"
Cohesion: 0.16
Nodes (23): analyzeVisualContrast(), analyzeVisualContrastCandidate(), checkElementColors(), checkElementColorsDOM(), checkElementGlowDOM(), checkElementIconTile(), checkElementIconTileDOM(), checkIconTile() (+15 more)

### Community 39 - "Context Builder"
Cohesion: 0.16
Nodes (17): appendOriginToDirective(), buildTagBlock(), commentClose(), commentOpen(), CONFIG_PATH, __dirname, ensureLiveGitIgnores(), escapeRegExp() (+9 more)

### Community 40 - "AI Analysis API"
Cohesion: 0.10
Nodes (23): needsTemplateUpdate(), POST(), POST(), cinematicLine(), cinematicValue(), POST(), resolvePost(), POST() (+15 more)

### Community 41 - "Color Anti-Pattern Checks"
Cohesion: 0.11
Nodes (13): fetchLinkedInSuggestions(), formatPeriod(), LIMITATIONS_SUGGESTIONS, linkedInResourcesStorageKey(), loadLinkedInResources(), OBJECTIVE_SUGGESTIONS, OFFERS_SUGGESTIONS, PLATFORMS (+5 more)

### Community 42 - "Calendar Post Utilities"
Cohesion: 0.10
Nodes (29): PATCH(), buildPostData(), mapPost(), parseDate(), buildConcreteReferenceCompositionBlock(), POST(), resolvePost(), stripRatioMentions() (+21 more)

### Community 43 - "Impeccable Path Utils"
Cohesion: 0.15
Nodes (12): Assess What "Extraordinary" Means Here, For data-heavy interfaces, For functional UI, For performance-critical UI, For visual/marketing surfaces, Implement with Discipline, Iterate with Browser Automation, Performance rules (+4 more)

### Community 44 - "Export & Google Drive API"
Cohesion: 0.19
Nodes (11): POST(), GET(), buildExportRows(), EXPORT_COLUMNS, generateCsv(), generatePdf(), generateXlsx(), PDF_FIXED_WIDTHS (+3 more)

### Community 46 - "Brand Identity Utils"
Cohesion: 0.13
Nodes (26): cinematicLine(), cinematicValue(), POST(), resolvePost(), parseAiJson(), POST(), buildUserInput(), needsTemplateUpdate() (+18 more)

### Community 47 - "SvelteKit Adapter"
Cohesion: 0.11
Nodes (33): firstExisting(), getDesignSidecarCandidates(), getDesignSidecarPath(), getImpeccableDir(), getLegacyLiveConfigPath(), getLegacyLiveServerPath(), getLegacyLiveSessionsDir(), getLiveAnnotationsDir() (+25 more)

### Community 48 - "Post Regeneration API"
Cohesion: 0.19
Nodes (18): args, buffer, cwd, pageUrlFilter, remaining, buildManualEditEvidence(), countByPage(), getBufferPath() (+10 more)

### Community 49 - "Post Regeneration (Legacy)"
Cohesion: 0.10
Nodes (14): APERTURE_OPTIONS, ASPECT_RATIOS, CAMERA_MOVEMENT_OPTIONS, CAMERA_OPTIONS, CINEMATIC_PRESETS, DURATIONS, FOCAL_LENGTH_OPTIONS, LENS_OPTIONS (+6 more)

### Community 50 - "Calendar Post CRUD & Image Prompt"
Cohesion: 0.09
Nodes (11): ASPECT_RATIOS, CREATIVE_GOALS, PLATFORMS, POST_TYPES, TEXT_DENSITIES, VISUAL_STYLES, ImagePromptModal(), PLATFORM_PRESETS (+3 more)

### Community 51 - "Eyebrow & H1 Anti-Pattern Checks"
Cohesion: 0.17
Nodes (11): Assess Animation Opportunities, Delight Moments, Entrance Animations, Feedback & Guidance, Implement Animations, Micro-interactions, Navigation & Flow, Plan Animation Strategy (+3 more)

### Community 52 - "Context Signals"
Cohesion: 0.10
Nodes (19): Clean Up, Code Quality, Color & Contrast, Content & Copy, Design System Discovery, Edge Cases & Error States, Final Verification, Forms & Inputs (+11 more)

### Community 53 - "Critique Storage"
Cohesion: 0.20
Nodes (9): Appropriate to Context, Assess Delight Opportunities, Compound Over Time, Delight Amplifies, Never Blocks, Delight Principles, Implementation Patterns, Register, Surprise and Discovery (+1 more)

### Community 54 - "Calendar Detail Page"
Cohesion: 0.18
Nodes (17): buildUpdateDirective(), cli(), compareSemver(), computeUpdateDirective(), DESIGN_NAMES, extractRegister(), FALLBACK_DIRS, fetchLatestSkillVersion() (+9 more)

### Community 55 - "CSS Typography Rules"
Cohesion: 0.12
Nodes (17): CSS Anchor Positioning, Destructive Actions: Undo > Confirm, Dropdown & Overlay Positioning, Fixed Positioning Fallback, Focus Rings: Do Them Right, Form Design: The Non-Obvious, Gesture Discoverability, Interaction Design (+9 more)

### Community 56 - "Cleanup Deprecated"
Cohesion: 0.14
Nodes (16): browserFindingsFromMap(), checkClippedOverflow(), checkCreamPalette(), checkElementClippedOverflow(), checkElementClippedOverflowDOM(), checkElementTextOverflowDOM(), checkHtmlPatterns(), checkPageQualityDOM() (+8 more)

### Community 57 - "Live Session Store"
Cohesion: 0.22
Nodes (5): Assess Current Layout, Live-mode signature params, Plan Layout Improvements, Register, Verify Layout Improvements

### Community 58 - "Brand Workspace Page"
Cohesion: 0.12
Nodes (15): Acceptance Criteria, Accessibility, Animations, Component: [ComponentName], Component Specification Template, Composition, Dependencies, Edge Cases (+7 more)

### Community 59 - "Brand Identity Extraction"
Cohesion: 0.14
Nodes (12): Impeccable OpenAI Agent Config, buildPresetInstruction(), PLATFORM_PRESET_LABELS, POST(), VALID_QUALITIES, VALID_SIZES, buildPresetInstruction(), PLATFORM_PRESET_LABELS (+4 more)

### Community 60 - "Image Generation & Higgsfield"
Cohesion: 0.33
Nodes (9): POST(), completeGeneratedMedia(), extractHiggsfieldExternalJobId(), extractHiggsfieldResultUrl(), generateImage(), getHiggsfieldClient(), refundTokens(), resolveHiggsfieldApiKey() (+1 more)

### Community 61 - "Overflow & Clipping Checks"
Cohesion: 0.12
Nodes (15): Assess Adaptation Challenge, Content Adaptation, Desktop Adaptation (Mobile → Desktop), Email Adaptation (Web → Email), Implement Adaptations, Layout Adaptation Techniques, Mobile Adaptation (Desktop → Mobile), Navigation Adaptation (+7 more)

### Community 62 - "CSP Detection"
Cohesion: 0.12
Nodes (15): Apply Clarity Principles, Assess Current Copy, Button & CTA Text, Confirmation Dialogs, Empty States, Error Messages, Form Labels & Instructions, Help Text & Tooltips (+7 more)

### Community 63 - "Voice Interface"
Cohesion: 0.12
Nodes (16): Avoid Redundant Copy, Confirmation Dialogs: Use Sparingly, Consistency: The Terminology Problem, Don't Blame the User, Empty States Are Opportunities, Error Message Templates, Error Messages: The Formula, Form Instructions (+8 more)

### Community 64 - "Live Server Event Management"
Cohesion: 0.12
Nodes (15): Anti-Goals, Brief Structure, Constraints, Content & Data, Design Direction, How to use the probes, Important limits, Interview cadence (+7 more)

### Community 65 - "Palette Builder"
Cohesion: 0.25
Nodes (8): Accessibility Considerations, Anti-reflexes worth defending against, Font Selection & Pairing, Pairing Principles, Reference Material, Typography, Typography System Architecture, Web Font Loading

### Community 66 - "Project Root & Pin"
Cohesion: 0.13
Nodes (14): 1. Accessibility (A11y), 2. Performance, 3. Theming, 4. Responsive Design, 5. Anti-Patterns (CRITICAL), Anti-Patterns Verdict, Audit Health Score, Detailed Findings by Severity (+6 more)

### Community 67 - "Content Calendar AI Generation"
Cohesion: 0.20
Nodes (15): blendRgba(), clampByte(), firstCssUrl(), getLayerValue(), loadVisualContrastImage(), parseObjectPosition(), parsePositionPair(), parsePositionToken() (+7 more)

### Community 68 - "Glow & Palette Anti-Patterns"
Cohesion: 0.23
Nodes (16): injectCli(), validateConfig(), applySvelteKitLiveAdapter(), buildSvelteLiveRootComponent(), defaultSvelteLayout(), detectSvelteKitProject(), ensureSvelteLiveRootComponent(), escapeRegExp() (+8 more)

### Community 69 - "Video From Brand Page"
Cohesion: 0.13
Nodes (14): 10. Architecture rules, 11. Claude Code low-token rules, 12. Current known risks, 13. Current development phase, 1. Project purpose, 2. Actual tech stack, 3. Folder map, 4. Important files (+6 more)

### Community 70 - "Live UI Core"
Cohesion: 0.25
Nodes (8): Break Card Grid Monotony, Choose the Right Layout Tool, Create Visual Rhythm, Establish a Spacing System, Improve Layout Systematically, Manage Depth & Elevation, Optical Adjustments, Strengthen Visual Hierarchy

### Community 71 - "Settings & AI Test Card"
Cohesion: 0.14
Nodes (13): Cleanup, Exit, Handle `accept`, Handle `discard`, Handle `manual_edit_apply`, Handle `prefetch`, Handle `steer`, Poll loop (+5 more)

### Community 72 - "Video Storyboard Modal"
Cohesion: 0.31
Nodes (5): defaultAspectRatio(), isVideoFormat(), PostSummary(), VIDEO_FORMATS, VideoStoryboardModal()

### Community 73 - "Browser Findings Overlay"
Cohesion: 0.14
Nodes (13): Animation Performance, Assess Performance Issues, Core Web Vitals Optimization, Cumulative Layout Shift (CLS < 0.1), First Input Delay (FID < 100ms) / INP (< 200ms), Largest Contentful Paint (LCP < 2.5s), Loading Performance, Network Optimization (+5 more)

### Community 74 - "Live Entry Point"
Cohesion: 0.23
Nodes (13): applyScopedPostRegeneration(), buildBrandSummary(), buildJsonHint(), buildJsonHintForCustomInstruction(), buildOutputImageTextRules(), buildOutputImageTextRulesForCustomInstruction(), CONTENT_FIELDS, createCompactToneInformationSummary() (+5 more)

### Community 75 - "Live Server Broadcast"
Cohesion: 0.23
Nodes (13): applyScopedPostRegeneration(), buildBrandSummary(), buildJsonHint(), buildJsonHintForCustomInstruction(), buildOutputImageTextRules(), buildOutputImageTextRulesForCustomInstruction(), CONTENT_FIELDS, createCompactToneInformationSummary() (+5 more)

### Community 76 - "Post Suggestion API"
Cohesion: 0.14
Nodes (11): AUDIENCE_LANGUAGE_SUGGESTIONS, CONTENT_STRATEGY_SUGGESTIONS, FORMATS, MAIN_GOAL_SUGGESTIONS, MONTHLY_SUBJECT_SUGGESTIONS, OFFER_SUGGESTIONS, PLATFORMS, StepCalendarForm() (+3 more)

### Community 77 - "Video Prompt Modal"
Cohesion: 0.32
Nodes (4): isVideoFormat(), PostSummary(), VIDEO_FORMATS, VideoPromptModal()

### Community 78 - "Generated Prompts Page"
Cohesion: 0.15
Nodes (12): Agent Selection, frontend-design-ui-ux, Goal, Guardrails, Inputs, Output Contract, Step 0: Discovery, Step 1: Model flows and states (+4 more)

### Community 79 - "Brand Identity Report"
Cohesion: 0.15
Nodes (11): Extract Flow, Step 1: Discover the Design System, Step 2: Identify Patterns, Step 3: Plan Extraction, Step 4: Extract & Enrich, Step 5: Migrate, Step 6: Document, Commands (+3 more)

### Community 80 - "Module Group 80"
Cohesion: 0.29
Nodes (7): adapter, __dirname, HIGGSFIELD_MODELS, main(), placeholder(), prisma, TEMPLATES

### Community 81 - "Module Group 81"
Cohesion: 0.24
Nodes (10): cli(), COMMON_DEV_PORTS, devServerSignals(), gatherSignals(), gitSignals(), hasCode(), latestCritique(), SCANNABLE_EXT (+2 more)

### Community 82 - "Module Group 82"
Cohesion: 0.31
Nodes (11): kebab(), listSnapshotsForSlug(), main(), nowFilenameStamp(), parseFrontmatter(), readLatestSnapshot(), readTrend(), serializeFrontmatter() (+3 more)

### Community 83 - "Module Group 83"
Cohesion: 0.18
Nodes (13): checkElementHeroEyebrow(), checkElementHeroEyebrowDOM(), checkElementOversizedH1(), checkElementOversizedH1DOM(), checkElementQuality(), checkElementQualityDOM(), checkHeroEyebrow(), checkOversizedH1() (+5 more)

### Community 84 - "Module Group 84"
Cohesion: 0.15
Nodes (11): Architecture, Before Editing, Claude Code Working Rules, Commands, Data flow, Large File Handling, Local file storage, Prisma 7 + SQLite setup (+3 more)

### Community 85 - "Module Group 85"
Cohesion: 0.22
Nodes (5): buildActivity(), formatTimeAgo(), HomePage(), pipeline, TYPE_ICON_MAP

### Community 86 - "Module Group 86"
Cohesion: 0.29
Nodes (7): Amplify the Design, Color Intensification, Composition Boldness, Motion & Animation, Spatial Drama, Typography Amplification, Visual Effects

### Community 87 - "Module Group 87"
Cohesion: 0.17
Nodes (11): Assess Current State, Code Simplification, Content Simplification, Document Removed Complexity, Information Architecture, Interaction Simplification, Layout Simplification, Plan Simplification (+3 more)

### Community 88 - "Module Group 88"
Cohesion: 0.06
Nodes (13): BrandDetailClient(), BrandIdentitySection(), COLOR_GROUPS, formatDate(), inkBtn, inputStyle, lbl, RADAR_AXES (+5 more)

### Community 89 - "Module Group 89"
Cohesion: 0.17
Nodes (11): Accessibility Resilience, Assess Hardening Needs, Edge Cases & Boundary Conditions, Error Handling, Hardening Dimensions, Input Validation & Sanitization, Internationalization (i18n), Performance Resilience (+3 more)

### Community 90 - "Module Group 90"
Cohesion: 0.29
Nodes (5): Assess Current Typography, Live-mode signature params, Plan Typography Improvements, Register, Verify Typography Improvements

### Community 91 - "Module Group 91"
Cohesion: 0.30
Nodes (11): buildTargetNames(), cleanSkillsLock(), cleanup(), DEPRECATED_NAMES, findProjectRoot(), findSkillsDirs(), HARNESS_DIRS, isImpeccableSkill() (+3 more)

### Community 92 - "Module Group 92"
Cohesion: 0.29
Nodes (3): DOW, lbl, TICK_COLOR

### Community 93 - "Module Group 93"
Cohesion: 0.13
Nodes (7): CalendarDetailClient(), EDIT_SECTIONS, REGEN_OPTIONS, STATUS_VARIANT, EXPORT_OPTIONS, extractFilename(), triggerDownload()

### Community 94 - "Module Group 94"
Cohesion: 0.26
Nodes (9): applyEvent(), baseSnapshot(), COMPLETED_PHASES, getJournalPath(), getSnapshotPath(), rebuildSnapshotFromJournal(), safeSessionId(), toPendingEvent() (+1 more)

### Community 95 - "Module Group 95"
Cohesion: 0.17
Nodes (12): Absolute bans, Color, Color & Theme, Copy, Design guidance, General rules, Interaction, Layout (+4 more)

### Community 96 - "Module Group 96"
Cohesion: 0.18
Nodes (10): Action Summary, Ask the User, Assessment A: Design Review, Assessment B: Detector + Browser Evidence, Assessment Orchestration, Hard Invariants, Persist the Snapshot, Purpose (+2 more)

### Community 97 - "Module Group 97"
Cohesion: 0.18
Nodes (11): 10. Help and Documentation, 1. Visibility of System Status, 2. Match Between System and Real World, 3. User Control and Freedom, 4. Consistency and Standards, 5. Error Prevention, 6. Recognition Rather Than Recall, 7. Flexibility and Efficiency of Use (+3 more)

### Community 98 - "Module Group 98"
Cohesion: 0.18
Nodes (11): 1. Read the screenshot (if present), 2. Wrap the element, 3. Load the action's reference, 5. Apply the freeform prompt (if present), 6. Write all variants in a single edit, 7. Parameters (composition-sized, 0–4 per variant), 8. Signal done, Aborting an in-flight session (+3 more)

### Community 99 - "Module Group 99"
Cohesion: 0.18
Nodes (10): Assess Current State, Color Refinement, Composition Refinement, Motion Reduction, Plan Refinement, Refine the Design, Register, Simplification (+2 more)

### Community 100 - "Module Group 100"
Cohesion: 0.20
Nodes (10): detectCsp(), INLINE_HEADER_SIGNALS, LAYOUT_EXTS, MONOREPO_HELPER_SIGNALS, NUXT_ROUTE_RULES_SIGNALS, NUXT_SECURITY_SIGNALS, SCAN_EXTS, SKIP_DIRS (+2 more)

### Community 101 - "Module Group 101"
Cohesion: 0.22
Nodes (9): Celebration Moments, Delight Techniques, Easter Eggs & Hidden Delights, Illustrations & Visual Personality, Loading & Waiting States, Micro-interactions & Animation, Personality in Copy, Satisfying Interactions (+1 more)

### Community 102 - "Module Group 102"
Cohesion: 0.24
Nodes (7): args, buildWeights(), hashUnit(), pickSeed(), seed, SEEDS, weightedPick()

### Community 103 - "Module Group 103"
Cohesion: 0.25
Nodes (9): __dirname, findHarnessDirs(), generatePinnedSkill(), HARNESS_DIRS, loadCommandMetadata(), pin(), root, unpin() (+1 more)

### Community 104 - "Module Group 104"
Cohesion: 0.20
Nodes (5): lbl, inkBtn, lbl, QUICK_ACTIONS, WorkspaceClient()

### Community 105 - "Module Group 105"
Cohesion: 0.22
Nodes (5): ExtractIdentityClient(), formatDate(), SavedIdentitySection(), SECTIONS, SplitIdentityView()

### Community 106 - "Module Group 106"
Cohesion: 0.29
Nodes (3): metadata, NAV, UTILITY

### Community 108 - "Module Group 108"
Cohesion: 0.13
Nodes (5): EDIT_SECTIONS, REGEN_OPTIONS, StepCalendarOutput(), Input(), Textarea()

### Community 109 - "Module Group 109"
Cohesion: 0.20
Nodes (10): Breakpoints: Content-Driven, Detect Input Method, Not Just Screen Size, Layout Adaptation Patterns, Mobile-First: Write It Right, Picture Element for Art Direction, Responsive Design, Responsive Images: Get It Right, Safe Areas: Handle the Notch (+2 more)

### Community 110 - "Module Group 110"
Cohesion: 0.50
Nodes (3): compilerOptions, paths, @/*

### Community 111 - "Module Group 111"
Cohesion: 0.20
Nodes (10): Craft Flow, Gates: do not compress, Production bar, Step 0: Project Foundation, Step 1: Shape the Design, Step 2: Load References, Step 3: Visual Direction & Assets (Harness-Gated), Step 4: Build to Production Quality (+2 more)

### Community 112 - "Module Group 112"
Cohesion: 0.20
Nodes (10): Anti-Patterns Verdict, Design Health Score, Generate Combined Critique Report, Minor Observations, Overall Impression, Persona Red Flags, Priority Issues, Questions to Consider (+2 more)

### Community 113 - "Module Group 113"
Cohesion: 0.20
Nodes (9): Color, Components, Layout, Motion, Product bans (on top of the shared absolute bans), Product permissions, Product register, The product slop test (+1 more)

### Community 114 - "Module Group 114"
Cohesion: 0.29
Nodes (10): checkColors(), checkElementAIPaletteDOM(), checkElementGlow(), checkGlow(), colorToHex(), contrastRatio(), getHue(), hasChroma() (+2 more)

### Community 115 - "Module Group 115"
Cohesion: 0.29
Nodes (8): appendStyleToLiveUiRoot(), appendToLiveUiRoot(), escapeCssIdent(), getLiveUiElementById(), LIVE_CHROME_MOUNT_CONTRACT, LIVE_UI_COMPONENT_IDS, LIVE_UI_SURFACES, resolveLiveUiRoot()

### Community 116 - "Module Group 116"
Cohesion: 0.36
Nodes (8): applyFallbackDates(), buildIdentitySummary(), createCompactToneInformationSummary(), deepFindPostArrays(), isPostLike(), parseAiJsonOutput(), parseToUtcDate(), POST()

### Community 117 - "Module Group 117"
Cohesion: 0.47
Nodes (3): platforms, Button(), buttonVariants

### Community 119 - "Module Group 119"
Cohesion: 0.38
Nodes (4): CATEGORY_LABELS, CATEGORY_ORDER, groupByCategory(), PromptLibraryClient()

### Community 120 - "Module Group 120"
Cohesion: 0.25
Nodes (8): Brand bans (on top of the shared absolute bans), Brand permissions, Brand register, Color, Imagery, Layout, Motion, The brand slop test

### Community 121 - "Module Group 121"
Cohesion: 0.22
Nodes (9): 1. The Wall of Options, 2. The Memory Bridge, 3. The Hidden Navigation, 4. The Jargon Barrier, 5. The Visual Noise Floor, 6. The Inconsistent Pattern, 7. The Multi-Task Demand, 8. The Context Switch (+1 more)

### Community 122 - "Module Group 122"
Cohesion: 0.25
Nodes (9): addBrowserFindings(), addVisualContrastFindings(), addVisualContrastResult(), clearOverlays(), detachOverlay(), disconnectLazyVisualContrastObserver(), scheduleLazyVisualContrast(), shouldRunVisualContrast() (+1 more)

### Community 123 - "Module Group 123"
Cohesion: 0.42
Nodes (7): __dirname, ensureServerRunning(), resolveFiles(), liveCli(), runScript(), safeParse(), scanForDrift()

### Community 124 - "Module Group 124"
Cohesion: 0.36
Nodes (7): buildIdentitySummary(), createCompactToneInformationSummary(), extractSeasonalDates(), normalisePost(), POST(), toHashtagArray(), toStringArray()

### Community 125 - "Module Group 125"
Cohesion: 0.25
Nodes (8): 1. Impatient Power User: "Alex", 2. Confused First-Timer: "Jordan", 3. Accessibility-Dependent User: "Sam", 4. Deliberate Stress Tester: "Riley", 5. Distracted Mobile User: "Casey", Persona-Based Design Testing, Project-Specific Personas, Selecting Personas

### Community 126 - "Module Group 126"
Cohesion: 0.25
Nodes (7): Init Flow, Step 1: Load current state, Step 2: Explore the codebase, Step 4: Write PRODUCT.md, Step 5: Decide on DESIGN.md, Step 6: Configure live mode (when code exists), Step 7: Recommend starting points, then wrap up

### Community 127 - "Module Group 127"
Cohesion: 0.36
Nodes (8): bindEditBadgeProxy(), editBadgeProxyTargets(), initEditBadgeHitProxies(), positionEditBadge(), setImportantStyle(), styleEditBadgeProxy(), syncEditBadgeHitProxies(), usesShadowChromeRoot()

### Community 128 - "Module Group 128"
Cohesion: 0.25
Nodes (8): Accessibility, CSS Animations, JavaScript Animation, Motion Materials, Perceived Performance, Performance, Technical Implementation, Timing & Easing

### Community 131 - "Module Group 131"
Cohesion: 0.39
Nodes (6): buildPalette(), deriveRadar(), extractHex(), extractName(), IdentityReportPage(), lbl

### Community 132 - "Module Group 132"
Cohesion: 0.32
Nodes (3): buildPostInfo(), ReferenceFlowClient(), STEPS

### Community 133 - "Module Group 133"
Cohesion: 0.29
Nodes (4): cell, filters, GeneratedPromptsClient(), lbl

### Community 134 - "Module Group 134"
Cohesion: 0.25
Nodes (7): 1. Folder responsibilities, 2. Data flow, 3. API structure, 4. Auth structure, 5. Business logic location, 6. Reusable component strategy, Architecture

### Community 135 - "Module Group 135"
Cohesion: 0.29
Nodes (7): Cognitive Load Assessment, Cognitive Load Checklist, Extraneous Load: Bad Design, Germane Load: Learning Effort, Intrinsic Load: The Task Itself, The Working Memory Rule, Three Types of Cognitive Load

### Community 136 - "Module Group 136"
Cohesion: 0.29
Nodes (7): Accessibility & Inclusion, Brand & Personality, Interview mode, not confirmation mode, Minimum viable interview, Register (ask first; it shapes everything below), Step 3: Ask strategic questions (for PRODUCT.md), Users & Purpose

### Community 137 - "Module Group 137"
Cohesion: 0.29
Nodes (7): append-arrays, append-string, Consent prompt template, CSP detection (first-time only), Drift-heal warning, First-time setup (config missing or invalid), Troubleshooting

### Community 138 - "Module Group 138"
Cohesion: 0.38
Nodes (4): buildVariables(), IMAGE_TEMPLATES, PostPromptClient(), VIDEO_TEMPLATES

### Community 139 - "Module Group 139"
Cohesion: 0.29
Nodes (6): 1. Existing styling approach, 2. Existing colours, 3. Existing typography, 4. Component style rules (observed), 5. TODO — open design-system items, Design System

### Community 140 - "Module Group 140"
Cohesion: 0.29
Nodes (6): 1. What appears already built, 2. What seems incomplete, 3. Suggested next development phases, 4. Known bugs or technical debt if visible, 5. Next safest steps, Roadmap

### Community 141 - "Module Group 141"
Cohesion: 0.33
Nodes (6): Font selection procedure, Pairing and voice, Reflex-reject aesthetic lanes, Reflex-reject list, Scale, Typography

### Community 142 - "Module Group 142"
Cohesion: 0.40
Nodes (6): checkRepeatedSectionKickers(), checkRepeatedSectionKickersDOM(), checkRepeatedSectionKickersFromDoc(), cleanInlineText(), collectRepeatedSectionKickerCandidates(), isRepeatedKickerCandidate()

### Community 143 - "Module Group 143"
Cohesion: 0.40
Nodes (6): clippedByInset(), clippedByRect(), expandBoxShorthand(), firstMetricLengthPx(), isScreenReaderOnlyTextStyle(), metricLengthPx()

### Community 145 - "Module Group 145"
Cohesion: 0.60
Nodes (5): buildCalendarPostSummary(), cinematicLine(), cinematicValue(), POST(), resolvePost()

### Community 147 - "Module Group 147"
Cohesion: 0.12
Nodes (7): TARGET_TOOLS, CATEGORY_ORDER, Badge(), badgeVariants, Label(), Separator(), TEMPLATE_VARIABLES

### Community 148 - "Module Group 148"
Cohesion: 0.33
Nodes (5): 1. Existing features (appear functional), 2. Partially built features, 3. Missing features, 4. Future feature ideas (if obvious from the codebase's trajectory), Features

### Community 151 - "Module Group 151"
Cohesion: 0.40
Nodes (5): 4. Plan three variants: identity first, then mode, then axes, Phase A: Extract the identity (non-skippable), Phase B: Pick mode (default vs departure), Phase C: Plan three variants, Phase D: Squint test

### Community 152 - "Module Group 152"
Cohesion: 0.40
Nodes (5): Handle fallback, Step 1: Identify where the element actually lives, Step 2: Show three variants in the DOM for preview, Step 3: On accept, write to true source, Step 4: On discard, clean up the served file

### Community 153 - "Module Group 153"
Cohesion: 0.50
Nodes (5): borderWidthsFromStyle(), checkElementGptBorderShadow(), checkElementGptBorderShadowDOM(), checkGptThinBorderWideShadow(), shadowMaxBlurPx()

### Community 154 - "Module Group 154"
Cohesion: 0.40
Nodes (4): 1. Technical decisions visible in the project, 2. Reasons (where stated or strongly inferable), 3. Open questions, Decisions

### Community 155 - "Module Group 155"
Cohesion: 0.50
Nodes (4): Heuristics Scoring Guide, Issue Severity (P0–P3), Reference Material, Score Summary

### Community 156 - "Module Group 156"
Cohesion: 0.50
Nodes (3): candidates, detectorPath, __dirname

### Community 162 - "Community 162"
Cohesion: 0.67
Nodes (4): postSerializedFindings(), renderBrowserFindings(), scanResultMeta(), serializeFindings()

### Community 163 - "Community 163"
Cohesion: 0.50
Nodes (3): Deploy on Vercel, Getting Started, Learn More

### Community 179 - "Community 179"
Cohesion: 0.25
Nodes (8): Animate complex properties, Interact with the device, Make data feel alive, Make transitions feel cinematic, Push performance boundaries, Render beyond CSS, The Toolkit, Tie animation to scroll position

### Community 180 - "Community 180"
Cohesion: 0.33
Nodes (6): Establish Hierarchy, Fix Readability, Font Selection, Improve Typography Systematically, Refine Details, Weight Consistency

### Community 181 - "Community 181"
Cohesion: 0.40
Nodes (4): Assess Current State, Plan Amplification, Register, Verify Quality

### Community 182 - "Community 182"
Cohesion: 0.50
Nodes (4): Classic Typography Principles, Modular Scale & Hierarchy, Readability & Measure, Vertical Rhythm

### Community 183 - "Community 183"
Cohesion: 0.50
Nodes (4): Fluid Type, Modern Web Typography, OpenType Features, Rendering polish

## Knowledge Gaps
- **852 isolated node(s):** `DEPRECATED_NAMES`, `HARNESS_DIRS`, `SKILL_FINGERPRINTS`, `COMMON_DEV_PORTS`, `SCANNABLE_EXT` (+847 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **26 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `confirm()` connect `Live Browser Param Management` to `Live Browser UI Rendering`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `Impeccable OpenAI Agent Config` connect `Brand Identity Extraction` to `AI Analysis API`, `Module Group 124`, `Brand Identity Report`?**
  _High betweenness centrality (0.082) - this node is a cross-community bridge._
- **Why does `pickVariantContent()` connect `Manual Edit Evidence Analysis` to `Live Browser Core`, `Live Svelte Component System`, `CSS Element Quality Rules`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Are the 25 inferred relationships involving `cn()` (e.g. with `Badge()` and `Button()`) actually correct?**
  _`cn()` has 25 INFERRED edges - model-reasoned connections that need verification._
- **What connects `DEPRECATED_NAMES`, `HARNESS_DIRS`, `SKILL_FINGERPRINTS` to the rest of the system?**
  _859 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Live Browser Core` be split into smaller, more focused modules?**
  _Cohesion score 0.03557951482479784 - nodes in this community are weakly interconnected._
- **Should `Live Server Core` be split into smaller, more focused modules?**
  _Cohesion score 0.04136874361593463 - nodes in this community are weakly interconnected._