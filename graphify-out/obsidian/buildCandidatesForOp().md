---
source_file: ".agents/skills/impeccable/scripts/live-manual-edit-evidence.mjs"
type: "code"
community: "Manual Edit Evidence Analysis"
location: "L132"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Manual_Edit_Evidence_Analysis
---

# buildCandidatesForOp()

## Connections
- [[analyzeSourceHint()]] - `calls` [EXTRACTED]
- [[findContextMatches()]] - `calls` [EXTRACTED]
- [[findLiteralMatches()]] - `calls` [EXTRACTED]
- [[findLocatorMatches()]] - `calls` [EXTRACTED]
- [[findObjectKeyMatches()]] - `calls` [EXTRACTED]
- [[literalMatchLimit()]] - `calls` [EXTRACTED]
- [[live-manual-edit-evidence.mjs]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Manual_Edit_Evidence_Analysis