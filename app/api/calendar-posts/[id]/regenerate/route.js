...
// ── Brand summary ─────────────────────────────────────────────────────────────
// Regeneration (like initial generation) needs BOTH visual identity (for visual
// direction fields) and tone/brand data (for hooks, captions, angles) — build a
// balanced summary covering both sections rather than dumping raw JSON.
...
// ── outputImageTextRequirements rules preamble (used by all scopes) ───────────
// Inserted before the JSON schema so AI has the context to produce quality,
// strategically-derived output. Mirrors the OUTPUT IMAGE TEXT REQUIREMENTS
// GENERATION RULES used by initial generation — same structured schema, same
// derivation hierarchy — so regeneration never produces weaker results than
// the original generation pass (Part 9: must use the SAME stronger rules).
