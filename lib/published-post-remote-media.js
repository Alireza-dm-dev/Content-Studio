...
// Budget assumes a conservative ~16 KB/s lower bound (measured SFTP throughput
// to the public media server ranged ~33-150 KB/s during testing). The previous
// formula assumed ~128 KB/s with no safety margin and capped at 10 minutes, so
// large media (videos, tens of MB uploaded over minutes) could equal or exceed
// the deadline and fail with SFTP_UPLOAD_TIMEOUT — while small images/PDFs
// (uploading in milliseconds) always succeeded. The higher cap keeps genuinely
// progressing large transfers from being killed by network jitter.
const UPLOAD_TIMEOUT_MAX_MS = 1800000;
...
export function calcUploadTimeout(fileSizeBytes) {
  // Ward off timeout failures on large media. Assume a conservative ~16 KB/s
  // lower bound (measured throughput ranged ~33-150 KB/s) so real transfers
  // finish well inside the budget even under network jitter. Small files still
  // clamp up to MIN_MS, so image/PDF uploads (which finish in milliseconds) are
  // unaffected.
  return Math.max(
    UPLOAD_TIMEOUT_MIN_MS,
    Math.min(
      UPLOAD_TIMEOUT_MAX_MS,
      60000 + Math.ceil(fileSizeBytes / (16 * 1024)) * 1000,
    ),
  );
}
...
async function uploadSingleFileWithRetry(cfg, client, rec, localPath, remotePath, fileName, remoteDir, folderName) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    // Give the retry attempt more headroom so a throttled/slow first attempt
    // (e.g. a large video) has a real second chance instead of re-hitting the
    // same short deadline and failing identically.
    const attemptTimeoutMs = isRetry
      ? Math.min(timeoutMs * 2, UPLOAD_TIMEOUT_MAX_MS * 2)
      : timeoutMs;

    const result = await attemptUpload(
      currentClient, localPath, remotePath, attemptTimeoutMs, rec, folderName, isRetry,
    );
