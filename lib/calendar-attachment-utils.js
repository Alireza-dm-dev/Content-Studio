...
export function validateCalendarAttachmentFile({ fileName, mimeType, sizeBytes }) {
  if (fileName === undefined || fileName === null || (typeof fileName === 'string' && !fileName.trim())) {
    return { ok: false, code: 'INVALID_FILENAME', error: 'File name is required' };
  }
  ...
  if (sizeBytes === undefined || sizeBytes === null) {
    return { ok: false, code: 'INVALID_SIZE', error: 'Invalid file size' };
  }
  ...
  if (sizeBytes <= 0) {
    return { ok: false, code: 'FILE_EMPTY', error: 'File is empty' };
  }
  if (sizeBytes > MAX_ATTACHMENT_FILE_SIZE_BYTES) {
    return { ok: false, code: 'FILE_TOO_LARGE', error: 'File exceeds the 5 MB limit' };
  }

  if (!ext || !isFormatSupported(ext)) {
    return { ok: false, code: 'UNSUPPORTED_TYPE', error: 'Unsupported file type' };
  }

  const normalizedMime = normalizeMimeType(mimeType);
  if (!isMimeValidForExtension(ext, normalizedMime)) {
    return { ok: false, code: 'MIME_MISMATCH', error: 'File extension and MIME type do not match' };
  }
...
export function decodeAttachmentBuffer(buffer) {
  if (!(buffer instanceof Uint8Array || Buffer.isBuffer(buffer))) {
    return { ok: false, code: 'INVALID_CONTENT', error: 'File content is not valid readable text' };
  }
  ...
  // Bytes decoded fine but there is nothing printable — this is a distinct
  // case from genuinely unreadable/binary content (Part 5 requirement).
  const printable = text.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]/g, '').trim();
  if (!printable) {
    return { ok: false, code: 'NO_TEXT_EXTRACTED', error: 'No readable text could be extracted from this file.' };
  }
...
function extractJson(text) {
  ...
  try {
    ...
    return { ok: false, code: 'INVALID_JSON', error: 'Invalid JSON document' };
  }
...
async function extractPdfText(bytes, validation) {
  ...
  if (signature !== '%PDF-') {
    return { ok: false, code: 'INVALID_PDF', extractionStatus: 'failed', error: 'Invalid PDF file' };
  }
  ...
  try {
    ...
    if (/password|encrypt|encrypted/i.test(msg)) {
      return { ok: false, code: 'PDF_ENCRYPTED', extractionStatus: 'failed', error: 'This PDF is encrypted and cannot be processed.' };
    }
    return { ok: false, code: 'INVALID_PDF', extractionStatus: 'failed', error: 'This PDF does not contain extractable text. Upload a text-based PDF.' };
  }
  ...
  if (!normalized.trim()) {
    return { ok: false, code: 'NO_TEXT_EXTRACTED', extractionStatus: 'failed', error: 'This PDF does not contain extractable text. Upload a text-based PDF.' };
  }
...
async function extractDocxText(bytes, validation) {
  ...
  if (magic !== 'PK\u0003\u0004' && magic.charCodeAt(0) !== 0x50) {
    return { ok: false, code: 'INVALID_DOCX', extractionStatus: 'failed', error: 'Invalid DOCX file.' };
  }
  ...
  try {
    ...
    if (/password|encrypt|protected/i.test(msg)) {
      return { ok: false, code: 'DOCX_PROTECTED', extractionStatus: 'failed', error: 'This DOCX file is password-protected and cannot be processed.' };
    }
    return { ok: false, code: 'INVALID_DOCX', extractionStatus: 'failed', error: 'Invalid or corrupted DOCX file.' };
  }
  ...
  if (!normalized.trim()) {
    return { ok: false, code: 'NO_TEXT_EXTRACTED', extractionStatus: 'failed', error: 'No usable text found in this DOCX file.' };
  }
...
export async function extractCalendarAttachmentText({ buffer, fileName, mimeType, sizeBytes }) {
  ...
  if (!validation.ok) {
    return { ok: false, code: validation.code, extractionStatus: 'failed', error: validation.error };
  }

  if (!buffer || !(buffer instanceof Uint8Array || Buffer.isBuffer(buffer))) {
    return { ok: false, code: 'INVALID_CONTENT', extractionStatus: 'failed', error: 'File content is not valid readable text' };
  }

  const bytes = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer);

  if (bytes.length > MAX_ATTACHMENT_FILE_SIZE_BYTES) {
    return { ok: false, code: 'FILE_TOO_LARGE', extractionStatus: 'failed', error: 'File exceeds the 5 MB limit' };
  }
  ...
  if (sizeBytes) {
    ...
    if (diff > tolerance) {
      return { ok: false, code: 'SIZE_MISMATCH', extractionStatus: 'failed', error: 'Declared file size and actual content size do not match' };
    }
  }
  ...
  if (!decoded.ok) {
    return { ok: false, code: decoded.code, extractionStatus: 'failed', error: decoded.error };
  }
  ...
  if (ext === '.txt' || ext === '.md' || ext === '.markdown') {
    ...
    if (!jsonResult.ok) {
      return { ok: false, code: jsonResult.code, extractionStatus: 'failed', error: jsonResult.error };
    }
