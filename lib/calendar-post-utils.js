...
// Removes a trailing run of hashtag tokens — defends against the AI embedding
// its own hashtags despite being told not to, whether on their own line or
// (observed in practice) tacked onto the end of the CTA sentence on the same
// line — which would otherwise leave hashtags appearing twice once we append
// ours. Operates on the raw string (not line-by-line) so it catches both.
const TRAILING_HASHTAGS_RE = /(?:[ \t]*\n\s*|[ \t]+)(?:#[\w؀-ۿЀ-ӿ]+(?:[ \t]+|$))+$/;

function stripTrailingHashtags(caption) {
  const text = String(caption ?? "").replace(/\s+$/, "");
  return text.replace(TRAILING_HASHTAGS_RE, "").replace(/\s+$/, "");
}

export function mergeHashtagsIntoCaption(caption, hashtagsRaw) {
  const hashtags = normalizeHashtagList(hashtagsRaw);
  const base = stripTrailingHashtags(caption);
...
export const CAPTION_GENERATION_RULES_LINES = [
  ...
  "  - Return exactly 5 hashtags, each starting with #, no more and no fewer.",
  "  - Hashtags must be relevant to the specific post, the brand, the topic, and the platform — avoid generic/spam hashtags (e.g. #love #instagood #follow) unless genuinely relevant to this post.",
];
