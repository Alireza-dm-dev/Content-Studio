...
const OITR_FIELD_KEYS = [
  "main_headline", "subheadline", "supporting_text", "call_to_action",
...
function _fieldsToDisplayString(fields) {
  ...
  return `{${keys.map(k => `${k}: '${cleaned[k]}'`).join(", ")}}`;
}
  ...
  if (structured) {
    if (structured.type === "carousel") {
      const lines = structured.slides
        ...
        .map(s => {
          ...
          return body ? `Slide ${s.slideNumber}: ${body}` : null;
        })
        .filter(Boolean);
    ...
    const lines = items
      .map((fields, i) => {
        ...
        if (!body) return null;
        return items.length > 1 ? `Image ${i + 1}: ${body}` : body;
      })
      .filter(Boolean);
    return lines.length ? lines.join("\n\n") : null;
  }
