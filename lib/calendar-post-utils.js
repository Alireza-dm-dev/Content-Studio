...
// Accepts either an array of tags or a single delimited string. A STRING is
// split on whitespace/commas first — the same delimiter convention the routes
// and grid editors already use — because a stored `hashtags` column holds all
// tags in one space-separated string; treating it as a single tag used to
// collapse "#A #B #C" into "#A#B#C" and, on Instagram, then look like a
// one-hashtag post that needed four fallback tags derived for it.
//
// An ARRAY element stays one tag: internal whitespace is removed rather than
// split on, so "#Manchester Training" remains "#ManchesterTraining".
function toHashtagTokens(rawList) {
  if (Array.isArray(rawList)) return rawList;
  if (typeof rawList === "string") return rawList.split(/[\s,]+/);
...
export function normalizeHashtagList(rawList) {
  const seen = new Set();
  const out = [];
  const list = toHashtagTokens(rawList);
...
// Re-attaches an EXISTING hashtag list to a rewritten caption without touching
// the list itself: no derivation, no padding, no cap, no reordering, no case
// change. Used by scopes that rewrite caption text but have no mandate over
// hashtags — enrichment deepens a post's wording, it does not restrategise its
// discovery tags, so a 3-hashtag post stays a 3-hashtag post even on Instagram.
//
// The caption body is still scrubbed of hashtags first, so anything the model
// wrote inline cannot survive alongside (or instead of) the preserved list.
export function preserveHashtagsInCaption(caption, rawHashtags) {
  const hashtags = toHashtagTokens(rawHashtags)
    .map(raw => String(raw ?? "").trim().replace(/\s+/g, ""))
    .filter(Boolean)
    .map(tag => (tag.startsWith("#") ? tag : `#${tag}`));

  const base = stripAllHashtags(caption);
  if (!hashtags.length) return { caption: base, hashtags };
  return { caption: base ? `${base}\n\n${hashtags.join(" ")}` : hashtags.join(" "), hashtags };
}
