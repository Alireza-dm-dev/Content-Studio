...
// Import the cinematic-style helpers so they are also locally bound in this
// module (a bare `export { x } from` re-export does NOT create a local binding,
// so calling `validateCinematicStyle(...)` below would throw ReferenceError).
// They are re-exported for existing consumers that import them from here.
import {
  CINEMATIC_STYLE_OPTIONS,
  getCinematicStyleById,
  validateCinematicStyle,
  normalizeCinematicStyle,
...
export const IMAGE_PRESETS = [
  ...
  },
  {
    ...
    // One-line summary of what this preset means visually — surfaced in prompts.
    characteristics: "A moody nocturnal look: low-angle night framing with neon practicals, shallow depth of field, cool modern grade, and film-still rendering. Reads as a frame from a stylish night thriller.",
    controls: {
  ...
  },
  {
    ...
    // One-line summary of what this preset means visually — surfaced in prompts.
    characteristics: "A polished commercial product presentation: centered hero subject, three-quarter view, soft studio-softbox lighting, clean neutral grade, and pristine material detail on a 4:5 canvas.",
    controls: {
  ...
  },
  {
    ...
    // One-line summary of what this preset means visually — surfaced in prompts.
    characteristics: "An authentic everyday scene: wide environmental framing at eye level, natural daylight, deep focus, photorealistic rendering, and honest neutral colour.",
    controls: {
  ...
  },
  {
    ...
    // One-line summary of what this preset means visually — surfaced in prompts.
    characteristics: "A magazine-documentary hybrid: rule-of-thirds framing, overcast diffused light, muted editorial grade, and honest documentary rendering with moderate depth.",
    controls: {
      composition: "rule-of-thirds",
      perspective: "three-quarter",
      lens: "standard-50mm",
  ...
  },
  {
    ...
    // One-line summary of what this preset means visually — surfaced in prompts.
    characteristics: "A scroll-stopping social ad layout: negative-space composition built for copy, even studio lighting, brand-led colour application, and clean commercial-product rendering.",
    controls: {
  ...
  },
  {
    ...
    // One-line summary of what this preset means visually — surfaced in prompts.
    characteristics: "A high-impact portrait: tight close-up detail, low angle, ultra-shallow depth of field, dramatic rim lighting, high contrast, and cinematic film-still texture.",
    controls: {
...
export function normalizeVisualControls(input) {
  ...
  // Cinematic style: validated via shared helper
  const rawCinematic = input?.cinematicStyle;
  const normalizedCinematic = typeof rawCinematic === "string"
    ? (rawCinematic === "auto" ? "auto" : validateCinematicStyle(rawCinematic))
    : "auto";
  out.cinematicStyle = normalizedCinematic ?? "auto";
...
// Apply a preset on top of the CURRENT control state. The preset sets the
// eight production controls, but must NOT silently wipe an explicitly chosen
// cinematicStyle (previously selecting a preset after a style reset it to
// undefined → the server treated it as "auto").
export function applyPreset(presetValue, previous) {
  const preset = IMAGE_PRESETS.find((p) => p.value === presetValue);
  const base = typeof previous === "object" && previous !== null
    ? { ...previous }
    : defaultVisualControls();
  if (!preset) return { ...base, ...defaultVisualControls(), cinematicStyle: base.cinematicStyle ?? "auto" };
  return { ...base, ...preset.controls, preset: preset.value };
}
