...
export function buildExportRows(dbPosts) {
...
// ── Excel (xlsx) ──────────────────────────────────────────────────────────────
export async function generateXlsx(rows, calendarInfo) {
  // Dynamic import keeps xlsx out of any client bundle
  const XLSX = await import("xlsx").then(m => m.default ?? m);

  const wb = XLSX.utils.book_new();

  // ── Sheets 1–4: one per tab view (Schedule, Content, Visual, Video Production)
  for (const view of CALENDAR_TABLE_VIEWS) {
    const headerRow = view.columns.map(c => c.label);
    const dataRows  = rows.map(row => view.columns.map(c => row[c.key] ?? ""));
    const ws = XLSX.utils.aoa_to_sheet([headerRow, ...dataRows]);

    ws["!cols"] = view.columns.map(({ key }) => ({ wch: WIDE_KEYS.has(key) ? 48 : 22 }));
...
export async function generatePdf(rows, calendarInfo, logoBuffer, logoMimeType) {
  ...
  for (const [i, view] of CALENDAR_TABLE_VIEWS.entries()) {
    ...
    const widths = view.columns.map(({ key }) => _pdfColWidth(key));
    if (!widths.some((w) => w === "*")) {
      widths[widths.length - 1] = "*";
    }

    const headerRow = view.columns.map((c) => ({ text: c.label, style: "pdfTh" }));
    const dataRows  = rows.map((row) =>
      view.columns.map((c) => ({ text: String(row[c.key] ?? ""), style: "pdfTd" }))
    );
