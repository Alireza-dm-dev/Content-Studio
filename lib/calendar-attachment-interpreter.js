...
function validateInterpretationInput({ fileName, mimeType, extractedText, extractionStatus }) {
  ...
  if (!mimeType || typeof mimeType !== 'string' || !mimeType.trim()) {
    return { ok: false, code: 'INVALID_MIME', error: 'File MIME type is required' };
  }
  if (!extractedText || typeof extractedText !== 'string' || !extractedText.trim()) {
    return { ok: false, code: 'NO_TEXT_EXTRACTED', error: 'Extracted text is empty' };
  }
  if (extractedText.length > MAX_EXTRACTED_CHARS_PER_FILE) {
    return { ok: false, code: 'TEXT_TOO_LONG', error: 'Extracted text exceeds the maximum allowed length' };
  }
  if (extractionStatus !== 'complete' && extractionStatus !== 'truncated') {
    return { ok: false, code: 'EXTRACTION_NOT_READY', error: 'Extraction status must be "complete" or "truncated"' };
  }
  return { ok: true };
}
...
export async function interpretCalendarAttachment({
  ...
  if (!validation.ok) {
    return { ok: false, code: validation.code, interpretationStatus: 'failed', error: validation.error };
  }
  ...
  try {
    ...
    // Distinguish setup/config failures (missing prompt template, missing
    // API key) from a genuine bad-output failure, without leaking internal
    // details (template slugs, stack traces) to the client.
    const msg = (err && err.message) || '';
    if (err && err.code === 'AI_NOT_CONFIGURED') {
      console.error('[calendar-attachment-interpreter] AI not configured:', msg);
      return {
        ok: false,
        code: 'AI_NOT_CONFIGURED',
        interpretationStatus: 'failed',
        error: 'AI is not configured for document interpretation. Add an OpenAI API key in Settings.',
      };
    }
    if (/prompt template not found/i.test(msg)) {
      console.error('[calendar-attachment-interpreter] Interpreter template missing:', msg);
      return {
        ok: false,
        code: 'INTERPRETER_NOT_CONFIGURED',
        interpretationStatus: 'failed',
        error: 'Reference file interpretation is not set up on this server yet. Contact an administrator.',
      };
    }
    console.error('[calendar-attachment-interpreter] Provider call failed:', msg);
    return {
      ok: false,
